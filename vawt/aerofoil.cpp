...
void viterna_corrigan(DataPoint p, DataPoint stall, double ar) {
    ...
    double kd =
        (stall.cd - cd_max * pow(sin(stall.alpha), 2)) / cos(stall.alpha);
    double kl = (stall.cl - cd_max * sin(stall.alpha) * cos(stall.alpha)) *
                sin(stall.alpha) / pow(cos(stall.alpha), 2);
    p.cl = cd_max / 2.0 * sin(2.0 * p.alpha) +
           kl * pow(cos(p.alpha), 2) / sin(p.alpha);
    p.cd = cd_max * pow(sin(p.alpha), 2) + kd * cos(p.alpha);
}

/**
 * @brief resample the whole dataset, such that for each row the same alpha
 * values are used
 *
 * @param set
 */
...
void AerofoilBuilder::transform_row(DataRow& row) {
    if (!this->_symmetric) {
        throw "aspect ratio correction for asymetric profiles is not "
              "implemented";
    }
    vector<double>& alpha = get<1>(row);
    vector<double>& cl = get<2>(row);
...
void AerofoilBuilder::add_data(DataRow data) {
    ...
    auto it = lower_bound(
        this->data.begin(), this->data.end(), re,
        [](const std::tuple<double, std::vector<double>, std::vector<double>,
                            std::vector<double>>& tuple,
           double value) { return std::get<0>(tuple) < value; });
...
shared_ptr<Aerofoil> AerofoilBuilder::build() {
    ...
    for (auto dataset : data) {
        for (auto datapoint : boost::range::combine(
                 get<1>(dataset), get<2>(dataset), get<3>(dataset))) {
